...
import com.dnapayments.presentation.stories.PausableScaleAnimation
...
class PausableProgressBar @JvmOverloads constructor(
    ...
    fun setMinWithoutCallback() {
        maxProgressView!!.setBackgroundResource(R.color.colorPrimary)
    ...
    fun setMaxWithoutCallback() {
        maxProgressView!!.setBackgroundResource(R.color.colorAccent)
        maxProgressView!!.visibility = View.VISIBLE
        if (animation != null) {
            animation!!.setAnimationListener(null)
            animation!!.cancel()
    ...
    private fun finishProgress(isMax: Boolean) {
        if (isMax) maxProgressView!!.setBackgroundResource(R.color.colorAccent)
